...
class JiraRest
  ...
  def get_versions project_key
    ...
    resp.each do |version_array| 
      p version_array['name']
      p version_array['releaseDate']
   
      
      
      not_nil = !(version_array['releaseDate'].is_a? NilClass) 
      p version_array['name'] if not_nil and time <= Time.parse(version_array['releaseDate']).strftime("%F")
    ...
    p version_name.empty?
    p version_name.sort_by { |a| a.split('.').map &:to_i }.first


    @actual_version = version_name.empty? ? nil : version_name.sort_by { |a| a.split('.').map &:to_i }.first
